...
class WSProdutoJsonController extends Controller
{
    public function listar(Request $request) {

        $exportar = Produto::select('PROD_ATIVO as status','PROD_DT_CORTE as data_corte')
                        ->where('PROD_CNPJ',$request->cnpj)
                        ->where('PROD_PRODUTO',$request->produto)->first();
        ...
        if($exportar==null) {
            $retorno = array(
                'status' => 'INATIVO',
                'mensagem' => 'O '. $produto .' nao esta liberado para o CNPJ '. $request->cnpj .' Entre em contato com o representante da b2finance.'
            );
        } else {
